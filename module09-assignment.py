...
# Your code here
sales_by_region_category = sales_df.groupby(['Region', 'Category'])['Total_Sales'].sum()
top_categories_by_region = sales_by_region_category.groupby(level=0).idxmax()
...
overall_total = product_totals.sum()

product_revenue_analysis = pd.DataFrame({
    'total_revenue': product_totals,
    'percentage': (product_totals / overall_total) * 100
})
...
total_revenue = sales_df['Total_Sales'].sum()
total_units = sales_df['Units'].sum()
avg_sale_value = sales_df['Total_Sales'].mean()
...
print("I would inrease marketing towards younger demographics")
print("I would focus on finding cheaper ways to create our products to increase profitability")